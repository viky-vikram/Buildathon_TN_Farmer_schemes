...
def health_check(config: app_config.AppConfig) -> dict[str, Any]:
    ...
    index_exists = (config.faiss_directory / "index.faiss").exists() and (config.faiss_directory / "index.pkl").exists()
    ...
    status = "healthy" if dataset_exists and index_exists and openai_configured and langsmith_valid else "degraded"
    return {
        "status": status,
        ...
        "filesystem": config.data_directory.exists() and config.faiss_directory.exists(),
    }
...
def test_health_check_success_and_degraded_states(isolated_config: app_config.AppConfig) -> None:
    ...
    scraper.save_schemes(SAMPLE_SCHEMES, isolated_config)
    (isolated_config.faiss_directory / "index.faiss").write_bytes(b"fake")
    (isolated_config.faiss_directory / "index.pkl").write_bytes(b"fake")
...
@pytest.mark.api
def test_faiss_rebuild_rules_and_mocked_build(monkeypatch: pytest.MonkeyPatch, isolated_config: app_config.AppConfig) -> None:
    """FAISS metadata controls rebuild decisions and embeddings are mocked."""
    ...
    class FakeFAISS:
        def save_local(self, path: str) -> None:
            ...
            (target / "index.pkl").write_bytes(b"fake-pkl")

        @classmethod
        def from_documents(cls, docs: list[Any], embeddings: Any) -> "FakeFAISS":
            assert docs
            assert embeddings is not None
            return cls()

    monkeypatch.setattr(rag_pipeline, "OpenAIEmbeddings", lambda model: Mock(model=model))
    monkeypatch.setattr(rag_pipeline, "FAISS", FakeFAISS)
    metadata = rag_pipeline.build_faiss_index(SAMPLE_SCHEMES, isolated_config)
    assert metadata["chunk_count"] > 0
    assert rag_pipeline.index_requires_rebuild(SAMPLE_SCHEMES, isolated_config) is False

    changed_model = make_test_config(isolated_config.root_dir, data_directory=isolated_config.data_directory, faiss_directory=isolated_config.faiss_directory, openai_embedding_model="other-model")
    assert rag_pipeline.index_requires_rebuild(SAMPLE_SCHEMES, changed_model) is True


@pytest.mark.api
def test_load_faiss_uses_trusted_local_deserialization(monkeypatch: pytest.MonkeyPatch, isolated_config: app_config.AppConfig) -> None:
    """FAISS loading uses the trusted local app directory and explicit dangerous-deserialization flag."""
    ...
    class FakeFAISS:
        @staticmethod
        def load_local(path: str, embeddings: Any, allow_dangerous_deserialization: bool) -> str:
            calls["path"] = path
            calls["flag"] = allow_dangerous_deserialization
            calls["embeddings"] = embeddings
            return "loaded"

    monkeypatch.setattr(rag_pipeline, "OpenAIEmbeddings", lambda model: Mock(model=model))
    monkeypatch.setattr(rag_pipeline, "FAISS", FakeFAISS)
    assert rag_pipeline.load_faiss_index(isolated_config) == "loaded"
    assert calls["path"] == str(isolated_config.faiss_directory)
